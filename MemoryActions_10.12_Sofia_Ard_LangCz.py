...
# Kamil
LANG = {
    'text_instr2':u"Pokračujte stisknutím tlačíka 'A'", 
    'signal':"teď",
    'question':"Který objekt byl blíže ke křížku?\n\n'A'   trojúhelník\n'B'   čtverec",
    'text_end_block':u"Pokračujte stistnutím tlačíka 'A'\n\n",
    'text_thanks':u"Experiment skončil.\n\nDěkujeme vám za účast!",
    'textInstr1':u"Pohněte páčkou k bližšímu kroužku",
    'textInstr2':u"Pohněte páčkou k bližšímu objektu a také si zapamatujte, který objekt to byl: trojúhelník nebo čtverec",
    'textInstr3':u"Zapamatujte si, který kroužek byl blíže ke křížku. \n\nKdyž uvidíte signál (slovo 'teď'), pohněte páčkou směrem k jeho pozici.",
    'textInstr4':u"Zapamatujte si, který objekt byl blíže ke křížku a který to je: trojúhelník nebo čtverec. \n\nKdyž uvidíte signál (slovo 'teď'), pohněte páčkou směrem k jeho pozici.",
    'msqFeedback1':"Správně",
    'msqFeedback2':"Špatně",
    'textScore1':u"Skóre",
    'textScore2':u"správně",
    'textScore3':"z",
    'textScore4':u"pokusů",
    'textScore5':u"čas odpovědi",
    'textScore6':u"vynechané pokusy",
    'textScore7':u"správných odpovědí na otázku",
    'pause':u"Počtejte ještě"
}
#----------------------------->
...
for thisTrial in trials:
    ...
    for thisLoopPause in loopPause:
        ...
        if immed==1 and delayed_d==0:
            textInstr = LANG['textInstr1']
        elif immed==1 and delayed_d==1:
            textInstr = LANG['textInstr2']
        elif delayed==1 and delayed_d==0:
            textInstr = LANG['textInstr3']
        elif delayed==1 and delayed_d==1:
            textInstr = LANG['textInstr4']
    ...
    for thisLoopFeedback in loopFeedback:
        ...
        # update component parameters for each repeat
        if joystick_resp_corr == 1:
            msqFeedback=LANG['msqFeedback1']
        else:
            msqFeedback=LANG['msqFeedback2']
    ...
    for thisLoop_fb_question in loop_fb_question:
        ...
        if answer_button.corr:
            answFeedback=LANG['msqFeedback1']
        else:
            answFeedback=LANG['msqFeedback2']
    ...
    for thisLoopEnd_block in loopEnd_block:
        ...
        # update component parameters for each repeat       
        if attempts > 0:
            textScore= LANG['textScore1']+': ' + '\n' + str(sumScore) +  ' '+LANG['textScore2']+' ' 
            textScore += ( "(%.0f" % (sumScore/attempts*100)) + ' %) '+LANG['textScore3']+' ' + str(attempts) + ' '+LANG['textScore4']
            if attempts == missed:
                textScore += '\n' + LANG['textScore5']+': --';
            else:
                textScore += '\n' + LANG['textScore5']+': ' + ( "%.0f" %  (sumRt/(attempts-missed)*1000) ) + ' ms';
            textScore += '\n'+LANG['textScore6']+': ' +  str(missed); 
            if delayed_d ==1:     # ---------<  Sofiia 06.11.2020, adding the sum of correct answers
                textScore += '\n'  +  str(answScore) +  ' '+LANG['textScore7']+' ' 
